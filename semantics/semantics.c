...
static void semantic_error(struct bytecode *code, struct tree_node *root, char *fmt, ...);
...
emit_code(struct bytecode *code, struct tree_node *root)
{
        ...
        case NODE_AND_EXPR:
                ...
                if (jumplen > MAX_JUMP) {
                        semantic_error(code, root, "max jump size (%d) exceeded", MAX_JUMP);
                        return;
                }
        ...
        case NODE_INTGER_CONST:
                ...
                break;
        default:
                semantic_error(code, root, "semantic analysis for node not implemented (%s)", nodetypestring(root->type));
        }
}
...
static void
semantic_error(struct bytecode *code, struct tree_node *root, char *fmt, ...)
{
